...
void SPH::InitGPUResources()
{
	...
	D3D11_BUFFER_DESC outputDescRadix;
	outputDescRadix.Usage = D3D11_USAGE_DEFAULT;
	outputDescRadix.ByteWidth = numElements * sizeof(UINT);
	outputDescRadix.BindFlags = D3D11_BIND_UNORDERED_ACCESS;
	outputDescRadix.CPUAccessFlags = 0;
	outputDescRadix.StructureByteStride = sizeof(UINT);
	outputDescRadix.MiscFlags = D3D11_RESOURCE_MISC_BUFFER_STRUCTURED;
	device->CreateBuffer(&outputDescRadix, 0, &histogramBuffer);

	D3D11_UNORDERED_ACCESS_VIEW_DESC uavDescRadix;
	uavDescRadix.Buffer.FirstElement = 0;
	uavDescRadix.Buffer.Flags = 0;
	uavDescRadix.Buffer.NumElements = numElements;
	uavDescRadix.Format = DXGI_FORMAT_UNKNOWN;
	uavDescRadix.ViewDimension = D3D11_UAV_DIMENSION_BUFFER;
	hr = device->CreateUnorderedAccessView(histogramBuffer, &uavDescRadix, &histogramUAV);
...
void SPH::UpdateRadixSorting(float deltaTime)
{
	...
	cb.numGroups = std::ceil(NUM_OF_PARTICLES / THREADS_PER_GROUPs);
